...
public class PlaceDAO {
    ...
    public Place insert(Place place) {
    	...
    	cv.put(DATETIME_COLUMN, place.getDatatime());
        ...
        // 設定編號
    	place.setId(id);//再新增place之後才會產生ID
        // 回傳結果
    	return place;
    }
    ...
    public boolean update(Place place) {
    	...
    	return db.update(TABLE_NAME, cv, where, null)>0; //當有成功更新則會大於零,會回傳更新筆數
    }
    ...
    public boolean delete(long id){
        ...
        // 刪除指定編號資料並回傳刪除是否成功
    	return db.delete(TABLE_NAME, where, null)>0; //當有成功更新則會大於零,會回傳刪除筆數
    }
    ...
    public Cursor getSearchCursor(String date) {
        ...
        // 字串資料必須在前後加上「'」，數字不用
    	String where = "subdtr(datatime,1,10)='"+date+"'";//substr()是SQL語法
        // 查詢指定日期條件的資料
    ...
    public Place getRecord(Cursor cursor) {
    	...
    	result.setDatatime(cursor.getString(4));
    ...
    public void sampleData(Context context) {
    	SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);//得到一個參考物件
    	
    	boolean firstTime = sp.getBoolean("FIRST_TIME", true);//如果KEY值為0則為true
    	
    	if(firstTime){
    		Place p01 = new Place(0,25.04719,121.516981,10.0,"2011-12-31 08:30","Hello!");
    		Place p02 = new Place(0,25.04719,121.516981,10.0,"2011-12-31 08:30","Hi!");
    		Place p03 = new Place(0,25.04719,121.516981,10.0,"2011-12-31 08:30","Awesome");
    		...
    		for(int i=0;i<10;i++){
    			...
    			insert(place);//插入一個place
    		}
    		SharedPreferences.Editor editor = sp.edit();
    		editor.putBoolean("FIRST_TIME", false);////如果KEY值不為0則為false
    		editor.commit();
    	}
    }
